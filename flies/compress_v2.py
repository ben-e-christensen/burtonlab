...
SAFETY
    ...
    Problems are listed in DST/compress_log_<time>.csv (one per run).

    Several runs at once are safe: each sequence is claimed with a lock
    file first, so e.g. a --reverse run never duplicates the first one.

ORDER / SPEED
    Bursts (e###) go first, continuous frames (p###) last.
      --skip-cont   leave the continuous frames out (do them another day)
      --only-cont   only the continuous frames
      --reverse     from the end of the list - start a 2nd run with this
...
STALE_LOCK_S = 2 * 3600       # a lock older than this is from a dead run
...
def encode(seq, dst, ffmpeg, ffprobe):
    ...
    # claim it, so a second run (e.g. --reverse) never does the same one
    lock = out_dir / f'.{prefix}.lock'
    try:
        if lock.exists() and time.time() - lock.stat().st_mtime > STALE_LOCK_S:
            lock.unlink()                       # left behind by a dead run
        ...
        return {'status': 'busy'}
    try:
        return _encode(seq, d, prefix, files, out_dir, first, mono16,
                       lossless, ext, out, manifest, ffmpeg, ffprobe)
    finally:
        lock.unlink(missing_ok=True)


def _encode(seq, d, prefix, files, out_dir, first, mono16, lossless, ext,
            out, manifest, ffmpeg, ffprobe):
...
def compress(src, dst, skip_cont=False, only_cont=False, reverse=False):
    ...
    is_cont = lambda sq: sq['prefix'].startswith('p')
    if skip_cont:
        n = sum(map(is_cont, seqs))
        seqs = [sq for sq in seqs if not is_cont(sq)]
        print(f'[scan] --skip-cont: leaving {n:,} continuous sequences for '
              f'a later run')
    if only_cont:
        seqs = [sq for sq in seqs if is_cont(sq)]
        copies = []
    # bursts (and everything else) first, continuous frames last
    seqs.sort(key=lambda sq: (is_cont(sq), str(sq['rel']), sq['prefix']))
    if reverse:
        seqs.reverse()
    ...
    for i, (s, dd, size) in enumerate(copies):
        ...
        tmp = dd.with_name(f'{dd.name}.{os.getpid()}.tmp')
        shutil.copy2(s, tmp)
        os.replace(tmp, dd)               # atomic: safe with two runs
        n_copied += 1
        if (i + 1) % 500 == 0:
        ...
        while pending:
            ...
            for fut in finished:
                ...
                if res['status'] == 'ok':
                    ...
                    log(f'[{n_done}/{len(seqs)}] {name}: another run has it')
                else:
    ...
    log_path = dst / (f'compress_log_{time.strftime("%Y%m%d_%H%M%S")}'
                      f'_{os.getpid()}.csv')
    ...
    if errs:
        print(f'[done] {len(errs)} FAILED - see {log_path}; '
...
def main():
    ...
    ap.add_argument('--skip-cont', action='store_true',
                    help='leave continuous p### frames for a later run')
    ap.add_argument('--only-cont', action='store_true',
                    help='only the continuous p### frames')
    ap.add_argument('--reverse', action='store_true',
                    help='work from the end of the list (for a 2nd run)')
    ...
    compress(src, dst, args.skip_cont, args.only_cont, args.reverse)
