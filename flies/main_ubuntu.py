...
def find_serial_port():
    """Return an explicit SERIAL_PORT, or auto-detect a likely USB-serial device."""
    if SERIAL_PORT:
        return SERIAL_PORT
    ...
    if not candidates:
        raise RuntimeError(
            ...
            'Plug in the electrometer or set SERIAL_PORT explicitly.'
        )
    return candidates[0]
...
class ElectrometerApp:
    ...
    def start(self):
        ...
        self.acq_thread = AcquisitionThread(
            port, BAUDRATE, DELAY_MS, self.data_queue, self.filepath
        )
        ...
        self.status_var.set(f'Recording ({port}) -> {self.filepath.name}')
