...
CAM_OFFSET_S = {}       # e.g. {'brio1': -0.05} if a camera lags
...
def make_video(d, event, charge, events):
    cams = load_cams(d, event)
    if not cams:
        print(f'[event {event}] no camera bursts - skipped')
        return
    ...
    names = [master] + [n for n in CAM_FOLDERS if n in cams and n != master]
    ev = events.get(event)
